 ...
 * `emberkin.<apex>`, so the same call would emit `worlds.emberkin.<apex>` — the two-label shape
...
import { BASE, PUBLIC_PATHS, ROUTES, indexable, publicPath } from '../src/lib/routes.ts'
...
describe('the sitemap nginx serves', () => {
  it('names no hostname — every address is composed from $host', () => {
     ...
     * authoritative.
     */
    const xml = servedBody('/worlds/emberkin/sitemap.xml')
  ...
  it('lists every address this surface invites a crawler to', () => {
    const xml = servedBody('/worlds/emberkin/sitemap.xml')
  ...
  it('lists nothing else, and in particular not one player-only screen', () => {
    /*
     * The other direction, and the one that matters here. `/party`, `/satchel` and `/wardrobe` sit
     * behind `ProtectedRoute`: a crawler following any of them is handed a redirect to hub's
     * sign-in rather than a page, and what is behind it is one player's own Kin, items and
     * cosmetics. An entry for any of the three is an invitation this file cannot keep.
     */
    const xml = servedBody('/worlds/emberkin/sitemap.xml')
    const listed = [...xml.matchAll(/<loc>\$scheme:\/\/\$host([^<]*)<\/loc>/g)].map((m) =>
      m[1] === '' ? '/' : (m[1] ?? ''),
    )
    // PUBLIC_PATHS are ROUTER paths and a sitemap publishes PUBLIC ones, which since the nesting
    // are two different strings — `/dex` against `/worlds/emberkin/dex`. `publicPath` is the one
    // conversion between them, so comparing through it keeps this test asserting what it always
    // asserted (the sitemap lists exactly the public routes and nothing else) rather than
    // re-stating the mount a fourth time.
    assert.deepEqual([...listed].sort(), PUBLIC_PATHS.map(publicPath).sort())
    for (const gated of ['/party', '/satchel', '/wardrobe']) {
      assert.ok(!xml.includes(gated), `the sitemap invites a crawler to ${gated}, which needs a session`)
    }
  })
  ...
  it('is served as XML, because a sitemap sent as text/html is a sitemap nobody reads', () => {
    ...
    assert.match(
      nginx,
      /location = \/worlds\/emberkin\/sitemap\.xml \{[\s\S]*?types \{ \}[\s\S]*?default_type application\/xml;/,
    )
  })
  ...
  it('is derived from the route table rather than typed a fifth time', () => {
    ...
    // locations. This asserts the derivation above is real — a public route added there appears
    // here — and pins the ONE exception the derivation makes: `/` is protected and is published
    // anyway, because the estate's own sitemap already names this surface's front door and an
    // instruction that contradicts an invitation is worse than either alone.
    assert.deepEqual(PUBLIC_PATHS, ['/', '/dex', '/settings', '/credits'])
    assert.equal(ROUTES.find((r) => r.path === '/')?.protected, true)
    for (const gated of ['/party', '/satchel', '/wardrobe']) {
      assert.equal(ROUTES.find((r) => r.path === gated)?.protected, true)
    }
  })

  it('agrees with the robots directive the shell writes into the page', () => {
    /*
     * The sitemap is the invitation and `<meta name="robots">` is the instruction; the design
     * system's own sitemap module states that the two must not disagree. `DocumentMeta` in
     * components/shell.tsx asks `indexable()` for exactly the same answer this file lists, so the
     * assertion is that both read one declaration rather than that two lists happen to match.
     */
    for (const route of ROUTES) {
      assert.equal(
        indexable(route.path),
        PUBLIC_PATHS.includes(route.path),
        `${route.path} is invited by one of the two and refused by the other`,
      )
    }
    assert.equal(indexable('/nope'), false, 'an address this app does not own would be indexed')
  })
})
...
describe('an environment that is not mainnet', () => {
   ...
   *
   * A testnet Emberkin carries a test roster, a seeded world and deliberately broken encounters.
   * Indexed beside the real game, its pages are a dex and a wardrobe that look exactly like the
   * ones a player would spend money in — which makes this a support problem before it is an SEO
   * one.
   */
  ...
  it('refuses every crawler and serves no sitemap', () => {
    ...
    assert.match(nginx, /location = \/worlds\/emberkin\/sitemap\.xml \{[\s\S]*?if \(\$cf_env\) \{ return 404; \}/)
  })

  it('matches a suffixed subdomain as well as a bare environment apex', () => {
    // The environment is a SUFFIX on the first label now (`emberkin-testnet.`) and was an apex
...
describe('robots.txt — which this surface no longer serves', () => {
  it('is GONE from this repository, and its one line moved to the apex', () => {
    ...
    // `<apex>/worlds/emberkin`, so `<apex>/worlds/emberkin/robots.txt` is a file nothing will ever request — a
    ...
    // micro-site announces `https://$host/worlds/emberkin/sitemap.xml` from the one robots.txt this
...
describe('the security headers on the documents this file adds', () => {
  it('are repeated in both new locations, because add_header does not accumulate', () => {
    ...
    // ORIGIN ROOT and nowhere else, so `<apex>/worlds/emberkin/robots.txt` is a file nothing fetches, and
    ...
    for (const path of ['/worlds/emberkin/sitemap.xml']) {
  ...
  it('are repeated in /assets/ and in both static payload prefixes too', () => {
    // /assets/ is the location that serves the code; /art/ and /game/ are 65 MB of generated
    // images and glTF, and a `.glb` sniffed as something else is a renderer executing a guess.
    for (const prefix of ['/assets/', '/art/', '/game/'].map((p) => `${BASE}${p}`)) {
